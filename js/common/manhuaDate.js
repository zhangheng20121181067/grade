...
$(function() {
	$.fn.manhuaDate = function(options) {
		...
		//点击时间列表事件
		$("#calender td").stop().bind("click",function() {
			var dv = $(this).html();
			if (dv != "&nbsp;"){
				var str = "";
				if (options.isTime){
		...
		$("#year").stop().bind("click",function(){
			if(stc){
				clearTimeout(stc);//清除定时器
			}
		});
		//点击月选择下拉框的时候清除定时器阻止控件层关闭
		$("#month").stop().bind("click",function(){
			if(stc){
				clearTimeout(stc);//清除定时器
			}
		});
